                ...
                return (
                  <li key={i} className="flex flex-col items-center gap-2">
                    <button
                      ...
                      disabled={!isNavigable && !isActive}
                      aria-current={isActive ? "step" : undefined}
                      aria-label={`Step ${i + 1}: ${s.title}${isCompleted ? " (Completed)" : ""}`}
                      className={cn(
                        "group flex flex-col items-center gap-2 transition-all duration-300 outline-none",
                    ...
                    >
                      ...
                      <span className={cn(
                        "text-[8px] uppercase tracking-widest hidden md:block transition-colors duration-300",
                        isActive ? "text-white" : "text-zinc-600",
                        isNavigable && "group-hover:text-zinc-300"
                      )}>
                        {s.title}
                      </span>
                    </button>
