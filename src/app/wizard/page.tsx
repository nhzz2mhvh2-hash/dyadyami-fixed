                ...
                return (
                  <li key={i}>
                    <button
                      ...
                      aria-label={`${s.title} (Step ${stepNum} of ${steps.length})${isCompleted ? ", Completed" : isActive ? ", Current Step" : ""}`}
                      disabled={!canGoBack}
                      onClick={() => canGoBack && setStep(stepNum)}
                      className="flex flex-col items-center gap-2 group appearance-none bg-transparent border-none p-0 cursor-pointer disabled:cursor-not-allowed focus:outline-none"
                    >
                      <div className={cn(
