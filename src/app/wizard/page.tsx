    ...
    <main className="min-h-screen bg-black overflow-hidden flex flex-col text-white">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 pt-20">
        <div className="w-full max-w-4xl">
          {/* Progress Indicator */}
          <nav aria-label="Progress" className="w-full relative mb-12">
            ...
            <ol className="w-full flex justify-between">
              {steps.map((s, i) => {
                ...
                const canBacktrack = stepNum < step;
                ...
                return (
                  <li key={i} className="flex flex-col items-center gap-2">
                    <button
                      ...
                      onClick={() => canBacktrack && setStep(stepNum)}
                      aria-label={`${s.title} (Step ${stepNum} of ${steps.length})${isCompleted ? ' - Completed' : isActive ? ' - Active' : ''}`}
                      aria-current={isActive ? "step" : undefined}
                      className={cn(
                        "group appearance-none bg-transparent border-none p-0 flex flex-col items-center gap-2 focus-visible:outline-none",
