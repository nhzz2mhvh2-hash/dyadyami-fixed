          ...
          <nav aria-label="Progress" className="w-full mb-12 relative flex items-center">
            ...
            <ol className="w-full flex justify-between">
              {steps.map((s, i) => {
                ...
                const isInteractive = stepNum < step;
                ...
                return (
                  <li key={i}>
                    <button
                      ...
                      onClick={() => isInteractive && setStep(stepNum)}
                      aria-label={`${s.title} (Step ${stepNum} of ${steps.length})`}
                      aria-current={isCurrent ? "step" : undefined}
                      className="group flex flex-col items-center gap-2 cursor-pointer focus-visible:outline-none appearance-none bg-transparent border-none p-0 disabled:cursor-not-allowed"
                    >
                      <div className={cn(
                        ...
                        isCurrent ? "bg-white border-white scale-125 shadow-[0_0_15px_rgba(255,255,255,0.5)]" :
                        "bg-black border-zinc-800"
                      )} />
                      <span className={cn("text-[8px] uppercase tracking-widest hidden md:block", isCurrent ? "text-white" : "text-zinc-600")}>
                        {s.title}
                      </span>
                    </button>
                  </li>
