    ...
    <main className="min-h-screen bg-black overflow-hidden flex flex-col text-white">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 pt-20">
        <div className="w-full max-w-4xl">
          {/* Progress Indicator */}
          <nav aria-label="Expedition progress" className="mb-12 relative w-full">
            ...
            <ol className="flex justify-between w-full">
              {steps.map((s, i) => {
                ...
                const isNavigable = stepNum < step;

                return (
                  <li key={i}>
                    <button
                      type="button"
                      onClick={() => isNavigable && setStep(stepNum)}
                      disabled={!isNavigable}
                      aria-current={isCurrent ? "step" : undefined}
                      aria-label={`Step ${stepNum}: ${s.title}${isCompleted ? " (Completed)" : isCurrent ? " (Current)" : ""}`}
                      className="group flex flex-col items-center gap-2 cursor-pointer disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-luxury focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-full p-1"
                    >
                      <div
                        className={cn(
                      ...
                      <span
                        className={cn(
                          ...
                          isCurrent ? "text-white font-medium" : isCompleted ? "text-zinc-400 group-hover:text-white" : "text-zinc-600"
                        )}
                      >
                        {s.title}
                      </span>
