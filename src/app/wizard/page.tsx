    ...
    <main className="min-h-screen bg-black overflow-hidden flex flex-col text-white">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 pt-20">
        <div className="w-full max-w-4xl">
          {/* Progress Indicator */}
          <nav className="w-full mb-12 relative" aria-label="Progress">
            <div className="absolute top-[6px] left-0 w-full h-[1px] bg-white/5 -z-10" />
            <ol className="w-full flex justify-between">
              {steps.map((s, i) => {
                const isCompleted = step > i + 1;
                const isActive = step === i + 1;
                const canBacktrack = i + 1 < step;
                return (
                  <li key={i}>
                    <button
                      type="button"
                      onClick={() => canBacktrack && setStep(i + 1)}
                      disabled={!canBacktrack}
                      aria-label={`${s.title} (Step ${i + 1} of ${steps.length})${isCompleted ? ', Completed' : isActive ? ', Active' : ''}`}
                      aria-current={isActive ? "step" : undefined}
                      className={cn(
                        "group flex flex-col items-center gap-2 focus-visible:outline-none",
                        canBacktrack ? "cursor-pointer" : "cursor-default"
                      )}
                    >
                      ...
                      <span className={cn("text-[8px] uppercase tracking-widest hidden md:block", isActive ? "text-white" : "text-zinc-600")}>
                        {s.title}
                      </span>
                    </button>
                  </li>
